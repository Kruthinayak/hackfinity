...
def train():
    ...
    for epoch in range(EPOCHS):
        ...
        for images, labels in tqdm(train_dataloader, desc=f"Epoch {epoch+1}/{EPOCHS}"):
            ...
            # Filter out 'undefined' samples (label=2) — the model only has 2 output
            # classes (chihuahua=0, muffin=1). Passing label=2 to CrossEntropyLoss
            # causes: IndexError: Target 2 is out of bounds.
            # create_sampler(exclude_zero_weights=True) only drops zero-weight rows;
            # undefined samples with non-zero weights still slip through.
